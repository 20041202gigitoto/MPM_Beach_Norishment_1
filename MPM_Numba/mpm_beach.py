...
このファイルは mpm_scratch/mpm_beach.py の Numba 高速化版である。
物理モデル・パラメータ・出力は元のコードと同一で、1ステップ(substep)の
計算 (P2G -> 格子更新 -> G2P、SVDによるDrucker-Prager塑性リターンマッピング
を含む) を、numpyのバッチ演算ではなく Numba (@njit) でJITコンパイルされた
「粒子ごとの明示的forループ」に書き換えている点のみが異なる。

mpm_scratch版との違い
----------------------
- matmul2x2 / svd2x2 / _bspline_weights による (N,2,2) バッチ演算 + P2G/G2P
  の np.bincount 散布加算を、単一の @njit 関数 `_substep_kernel` に統合した
  「粒子ループ + 格子ループ」実装に置き換えた。Numbaはこのような明示的な
  スカラーループをネイティブコードにコンパイルするため、Pythonループの
  オーバーヘッドやnumpyの中間配列確保コストを避けられる。
- 初回の substep 呼び出し時にJITコンパイルが走るため、その1回だけ数秒程度
  余分に時間がかかる(2回目以降は高速なコンパイル済みコードが再利用される)。
- 数値結果は元のnumpy実装と数学的に同一の演算を行っており、同じ初期条件・
  同じ乱数を使わない決定論的な計算のため、実質的に同一の結果が得られる
  (浮動小数点演算の順序がわずかに異なるため、最終桁レベルの誤差は生じ得る)。
...
--------
    python mpm_beach.py
    python mpm_beach.py --t-total 2.0
...
# ---- 出力 ----
OUTPUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "output_beach")
...
# (F更新+Drucker-Prager塑性リターンマッピング -> P2G -> 格子更新 -> G2P) を
# 実行する。@njit によりネイティブコードへコンパイルされ、Pythonループの
# オーバーヘッドを避けつつ、numpyバッチ演算で必要だった大量の中間配列確保も
# 不要になる。SVDは2x2の閉形式解を直接スカラー演算で書き下している。
# ----------------------------------------------------------------------------
...
def _substep_kernel(x, v, C, F, mass, vol0, mu0, lambda0, alpha_dp,
    ...
    for p in range(n):
        ...
        # Kirchhoff応力 tau = U @ diag(principal_stress) @ U^T
...
def plot_results(snapshots, s: MPMState, out_dir):
    ...
    ax.set_title("沖合養浜マウンドの自重変形 (MPM, Numba)")
    ax.set_xlim(0, LX)
    ax.set_ylim(0, LY)
    ax.set_aspect("equal")
...
def _parse_args():
    ...
    parser.add_argument(
        ...
        help="出力先ディレクトリ(省略時は output_beach/。長時間実行では過去の出力を"
...
# ============================================================================
# - 波浪外力: mpm_beach_wave.py (Numba版) を参照。
# - 海底勾配: mpm_beach_wave_slope.py (Numba版) を参照。
# - 侵食・堆積の定量評価: mpm_beach_wave_slope_erosion.py (Numba版) を参照。
# - 形状関数の高次化(3次Bスプライン): mpm_beach_wave_cubic.py /
#   mpm_beach_wave_slope_cubic.py (Numba版) を参照。
# - さらなる高速化: `_substep_kernel` は @njit(parallel=True) 化して
#   `numba.prange` で粒子ループを並列化できる余地があるが、P2G の格子への
#   散布加算(grid_m/grid_vx/grid_vy への `+=`)は複数粒子が同じ格子ノードに
#   書き込むためデータ競合が生じ、単純な prange 化はできない(粒子ごとに
#   結果を一時バッファへ書き、後段でシリアルに集約する、あるいは
#   ノードごとに担当粒子を事前ソートするなどの追加設計が必要)。
