"""
mpm_beach_wave_slope_cubic.py (Numba版)
==========================================
mpm_beach_wave_slope.py (Numba版) をベースに、**形状関数を2次B-スプライン
から3次B-スプラインに変更した版**。mpm_scratch/mpm_beach_wave_slope_cubic.py
の Numba 高速化版であり、物理モデル(MLS-MPM本体、海底勾配、Drucker-Prager
砂塑性、波浪外力モデル、可視化)はすべて mpm_beach_wave_slope.py (Numba版)
と同一で、粒子-格子間の補間に使う形状関数の次数のみが異なる。

mpm_beach_wave_slope.py (Numba版) との違い
---------------------------------------------
- 形状関数: 2次B-スプライン(3x3=9近傍) から 3次B-スプライン(4x4=16近傍)に
  変更(詳細は mpm_beach_wave_cubic.py のコメントを参照)。
- APICの逆慣性係数 `d_inv`: 4/dx^2 -> 3/dx^2 (理論値)。
- 粒子座標のクリップ余白: DX -> 2*DX に拡大。
- `base` の計算式: `floor(x/dx - 0.5)` -> `floor(x/dx) - 1`。
- 出力先ディレクトリを `output_slope_cubic/` に分離した。
- substep 本体は Numba (@njit) でJITコンパイルされた粒子ごとの明示的
  forループとして実装している(numpyバッチ演算版からの変更点)。

実行方法
--------
    python mpm_beach_wave_slope_cubic.py
    python mpm_beach_wave_slope_cubic.py --t-total 12.0
...
# ---- 出力 ----
# 2次B-スプライン版(mpm_beach_wave_slope.py)の出力(output/)を上書きしないよう
# 別ディレクトリに分離する。
OUTPUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "output_slope_cubic")
...
# %%
# ============================================================================
# 3. MPM ソルバー本体 (MLS-MPM + Drucker-Prager 砂塑性 + 波浪外力, 海底勾配 + 3次Bスプライン)
# ============================================================================
...
def _cubic_w(d):
    """一様3次B-スプラインカーネルの1次元重み(距離 d = fx - offset に対して)。"""
    ad = abs(d)
    if ad < 1.0:
        return 2.0 / 3.0 - d * d + 0.5 * ad ** 3
    elif ad < 2.0:
        return (2.0 - ad) ** 3 / 6.0
    else:
        return 0.0
...
# ----------------------------------------------------------------------------
# Numba高速化カーネル: 3次B-スプライン版 + 海底勾配版(bottom_maskの値のみ
# 平坦版と異なる)。近傍数・base・d_inv・marginの規約は
# mpm_beach_wave_cubic.py のカーネルと同一。
# ----------------------------------------------------------------------------
...
def _substep_kernel(x, v, C, F, mass, vol0, mu0, lambda0, alpha_dp,
    ...
    for p in range(n):
        ...
        bi = int(np.floor(x[p, 0] * inv_dx)) - 1
        bj = int(np.floor(x[p, 1] * inv_dx)) - 1
        base_i[p] = bi
        base_j[p] = bj
        fxi = x[p, 0] * inv_dx - bi
        fyj = x[p, 1] * inv_dx - bj
        ...
        wxa = (_cubic_w(fxi - 0.0), _cubic_w(fxi - 1.0), _cubic_w(fxi - 2.0), _cubic_w(fxi - 3.0))
        wya = (_cubic_w(fyj - 0.0), _cubic_w(fyj - 1.0), _cubic_w(fyj - 2.0), _cubic_w(fyj - 3.0))
...
def plot_results(snapshots, s: MPMState, out_dir):
    ...
    ax.set_title("沖合養浜マウンドの自重変形+波浪外力による変形 (MPM, Numba, 海底勾配+3次B-スプライン)")
    ax.set_xlim(0, LX)
    ax.set_ylim(0, LY)
    ax.set_aspect("equal")
    ...
    ax.set_xlabel("岸沖距離 x [m]")
    ax.set_ylabel("高さ y [m]")
    ax.set_title("沖合養浜マウンド断面形状の変化(自重沈下・法面変形+波浪外力、海底勾配+3次B-スプライン)")
    ax.set_xlim(0, LX)
...
def _parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="MPM沖合養浜マウンド自重変形解析(Numba版, 海底勾配+3次B-スプライン)。"
    ...
    parser.add_argument(
        ...
        help="出力先ディレクトリ(省略時は output_slope_cubic/。長時間実行では"
             "過去の出力を上書きしないよう別ディレクトリを指定することを推奨)",
    )
...
# 拡張のヒント
# ============================================================================
# mpm_scratch/mpm_beach_wave_slope_cubic.py 末尾の「拡張のヒント」を参照
# (物理モデルの拡張ポイントはNumba化による変更の影響を受けない)。
