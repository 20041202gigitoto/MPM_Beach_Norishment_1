"""
mpm_beach_wave_cubic.py (Numba版)
===================================
mpm_beach_wave.py (Numba版) をベースに、**形状関数を2次B-スプラインから
3次B-スプラインに変更した版**。mpm_scratch/mpm_beach_wave_cubic.py の
Numba 高速化版であり、物理モデル(MLS-MPM本体、Drucker-Prager砂塑性、
波浪外力モデル、可視化)はすべて mpm_beach_wave.py (Numba版) と同一で、
粒子-格子間の補間に使う形状関数の次数のみが異なる。

mpm_beach_wave.py (Numba版) との違い
--------------------------------------
- 形状関数: 2次B-スプライン(サポート幅3ノード、1粒子あたり3x3=9近傍) から
  3次B-スプライン(サポート幅4ノード、1粒子あたり4x4=16近傍)に変更。
  `_cubic_bspline_weight()` を3次カーネル(区分3次多項式)として実装し、
  P2G/G2Pのオフセット・近傍数を 9 -> 16 に拡張した。
- APICの逆慣性係数 `d_inv`: 2次スプラインでは 4/dx^2 だったが、3次スプライン
  では理論値 3/dx^2 に変更(Jiang et al. 2016 の結果に基づく)。
- 粒子座標のクリップ余白: 3次スプラインはサポート幅が2次より1セル分広い
  ため、クリップ余白を DX -> 2*DX に拡大した。
- `base`(P2G/G2P用の基準ノード添字)の計算式: 2次では
  `floor(x/dx - 0.5)`、3次では `floor(x/dx) - 1` を用いる。
- 出力先ディレクトリを `output_cubic/` に分離した。
- substep 本体は Numba (@njit) でJITコンパイルされた粒子ごとの明示的
  forループとして実装している(numpyバッチ演算版からの変更点、詳細は
  mpm_beach.py 冒頭のコメントを参照)。

このファイルのそれ以外の内容(コード構成、パラメータ、docstring)は
mpm_beach_wave.py (Numba版) を踏襲している。

実行方法
--------
    python mpm_beach_wave_cubic.py
    python mpm_beach_wave_cubic.py --t-total 12.0
...
# %%
# ============================================================================
# 3. MPM ソルバー本体 (MLS-MPM + Drucker-Prager 砂塑性 + 波浪外力, 3次Bスプライン)
# ============================================================================
...
# ----------------------------------------------------------------------------
# Numba高速化カーネル: 3次B-スプライン版。近傍数が9->16に増える点、
# base/margin/d_invの規約が異なる点を除き、mpm_beach_wave.py の
# 同名カーネルとアルゴリズムは同一(コメントはそちらを参照)。
# ----------------------------------------------------------------------------
...
def _substep_kernel(x, v, C, F, mass, vol0, mu0, lambda0, alpha_dp,
    ...
    # 3次スプラインはサポート幅が4ノード分(2次版より1セル分広い)ため、
...
def plot_results(snapshots, s: MPMState, out_dir):
    ...
    ax.set_title("沖合養浜マウンドの自重変形+波浪外力による変形 (MPM, Numba, 3次B-スプライン)")
    ax.set_xlim(0, LX)
    ax.set_ylim(0, LY)
    ax.set_aspect("equal")
...
def _parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="MPM沖合養浜マウンド自重変形解析(Numba版, 3次B-スプライン)。"
...
# 拡張のヒント
# ============================================================================
# mpm_scratch/mpm_beach_wave.py の「拡張のヒント」節を参照(このファイルでは
# 形状関数の変更以外の拡張ポイントは変わらない)。
