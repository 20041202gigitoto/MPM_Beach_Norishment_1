"""
mpm_beach_wave_slope.py (Numba版)
====================================
mpm_beach_wave.py (Numba版) をベースに、**海底勾配(沖に向かって深くなる
海底地形)**を導入した版。mpm_scratch/mpm_beach_wave_slope.py の Numba
高速化版であり、物理モデル(MLS-MPM本体、Drucker-Prager砂塑性、波浪外力
モデル、可視化)はすべて mpm_beach_wave.py (Numba版) と同一で、海底地形の
扱いのみが異なる。substep 本体を Numba (@njit) の粒子ごとの明示的forループに
書き換えている点は mpm_beach_wave.py (Numba版) と共通(詳細は mpm_beach.py
冒頭のコメントを参照)。
...
- mpm_beach_wave.py は「局所的に平坦な海底(y=0)」を仮定していたが、本ファイル
  ...
  x位置ごとに切り替わる。
- それ以外(MLS-MPM本体、Drucker-Prager砂塑性、波浪外力モデル、Numba化された
  substep カーネルそのもの)は mpm_beach_wave.py (Numba版) と同一のロジック
  を流用している(境界条件を表す bottom_mask 配列の値が変わるだけで、
  カーネル内の演算は変わらない)。

実行方法
--------
    python mpm_beach_wave_slope.py
    python mpm_beach_wave_slope.py --t-total 12.0
...
# ---- 出力 ----
OUTPUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "output_slope")
...
# %%
# ============================================================================
# 3. MPM ソルバー本体 (MLS-MPM + Drucker-Prager 砂塑性 + 波浪外力, 海底勾配あり)
# ============================================================================
...
# ----------------------------------------------------------------------------
# Numba高速化カーネル: mpm_beach_wave.py (Numba版) の `_substep_kernel` と
# アルゴリズムは完全に同一(海底勾配は bottom_mask 配列の値としてのみ反映
# されており、カーネル内の演算式自体は変わらない)。
# ----------------------------------------------------------------------------
...
def plot_results(snapshots, s: MPMState, out_dir):
    ...
    ax.set_title("沖合養浜マウンドの自重変形+波浪外力による変形 (MPM, Numba, 海底勾配あり)")
    ax.set_xlim(0, LX)
    ax.set_ylim(0, LY)
    ax.set_aspect("equal")
    ...
    mpm_animation.save_particle_gif(
        ...
        title="沖合養浜マウンドの自重変形+波浪外力による変形 (MPM, Numba, 海底勾配あり)",
    )
...
def _parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="MPM沖合養浜マウンド自重変形解析(Numba版, 海底勾配あり)。"
    ...
    parser.add_argument(
        ...
        help="出力先ディレクトリ(省略時は output_slope/。長時間実行では過去の出力を"
...
# 拡張のヒント
# ============================================================================
# mpm_scratch/mpm_beach_wave_slope.py 末尾の「拡張のヒント」を参照
