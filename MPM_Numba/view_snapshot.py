...
view_snapshot.py
=================
mpm_beach.py (Numba版) が output/ に保存する snapshot_XXXX.npz の中身を
閲覧するための補助スクリプト。npz はNumPyのバイナリ形式なのでテキスト
エディタでは開けないため、このスクリプトで (1) 内容をテキスト表示、
(2) 断面図として可視化、(3) CSVへ変換、のいずれかを行う。
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="mpm_beach.py (Numba版) の snapshot npz を閲覧する")
