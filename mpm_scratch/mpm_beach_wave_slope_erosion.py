"""
mpm_beach_wave_slope_erosion.py
================================
mpm_beach_wave_slope.py の計算ロジック(MLS-MPM + Drucker-Prager砂塑性 +
簡易波浪外力)をそのまま再利用しつつ、侵食・堆積の過程を時系列で追跡できる
ようにした版(mpm_beach_wave_slope.py 末尾「拡張のヒント」の提案の実装)。
...
- dz(x, t) を x 方向に積分して、堆積量・侵食量・正味の断面積変化を
  フレームごとの時系列として記録する(2次元断面なので単位は m^2 =
  奥行き1mあたりの土砂体積に相当)。
...
- MPMソルバー本体(MPMState, substep, 粒子生成, bed_elevation など)は
  mpm_beach_wave_slope.py をそのままインポートして再利用し、物理ロジックの
  重複を避けている(パラメータを変えたい場合は mpm_beach_wave_slope.py 側の
  定数を編集すればこのファイルにもそのまま反映される)。
...
def surface_profile(x, y, x_max=base.LX, bin_width=ENVELOPE_BIN_WIDTH):
    ...
    これにより、初期状態でマウンドが存在しなかった位置(海底が露出していた
    位置)に後から砂が堆積した場合や、逆にマウンドが完全に洗掘されて
    海底が露出した場合でも、その変化が dz(x, t) や堆積量・侵食量の集計に
    正しく反映される(np.nanのままだと、そうした場所は「データなし」として
    集計から丸ごと除外されてしまい、マウンドが法面崩壊で当初の法尻より外側に
    広がる=堆積域が拡大するようなケースを過小評価してしまう)。
    """
...
def _parse_args():
    parser = argparse.ArgumentParser(
        description="MPM沖合養浜マウンド解析(海底勾配あり)+ 侵食・堆積の時系列追跡。"
    )
