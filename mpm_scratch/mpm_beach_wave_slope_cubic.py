"""
mpm_beach_wave_slope_cubic.py
==============================
mpm_beach_wave_slope.py をベースに、**形状関数を2次B-スプラインから3次
B-スプラインに変更した版**。物理モデル(MLS-MPM本体、海底勾配、
Drucker-Prager砂塑性、波浪外力モデル、可視化)はすべて
mpm_beach_wave_slope.py と同一で、粒子-格子間の補間に使う形状関数の
次数のみが異なる。

mpm_beach_wave_slope.py との違い
--------------------------------
...
- `base`(P2G/G2P用の基準ノード添字)の計算式: 2次では
  ...
  サポート範囲に対応した規約の違い)。
- 出力先ディレクトリを `output_slope_cubic/` に分離し、2次版
  (`mpm_beach_wave_slope.py`)の出力(`output/`)を上書きしないようにした。
...
このファイルのそれ以外の内容(コード構成、パラメータ、海底勾配モデル、
docstring)は mpm_beach_wave_slope.py を踏襲している。詳細な「このコードで
できること」「できないこと」等は mpm_beach_wave_slope.py の説明を参照。
...
# ---- 出力 ----
# 2次B-スプライン版(mpm_beach_wave_slope.py)の出力(output/)を上書きしない
# よう別ディレクトリに分離する。
...
def substep(s: MPMState, t: float):
    ...
    `d_inv` が異なる点を除き、2次版(mpm_beach_wave_slope.py)の
    substep() とアルゴリズムは同一。
...
def plot_results(snapshots, s: MPMState, out_dir):
    ...
    ax.set_title("沖合養浜マウンドの自重変形+波浪外力による変形 (MPM, 海底勾配あり, 3次B-スプライン)")
    ax.set_xlim(0, LX)
    ax.set_ylim(0, LY)
    ax.set_aspect("equal")
    ...
    ax.set_xlabel("岸沖距離 x [m]")
    ax.set_ylabel("高さ y [m]")
    ax.set_title("沖合養浜マウンド断面形状の変化(自重沈下・法面変形+波浪外力、海底勾配あり, 3次B-スプライン)")
    ax.set_xlim(0, LX)
...
def _parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="MPM沖合養浜マウンド自重変形解析(海底勾配あり, 3次B-スプライン)。"
    ...
    parser.add_argument(
        ...
        help="出力先ディレクトリ(省略時は output_slope_cubic/。長時間実行では過去の出力を"
...
# ============================================================================
# mpm_beach_wave_slope.py の「拡張のヒント」節を参照(このファイルでは
