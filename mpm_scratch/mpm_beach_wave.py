...
- 波の空間的な位相差(kx)は無視している(計算領域が波長に比べ短いことを
  踏まえた簡略化)。また対称な線形波のみを与えており、浅水変形に伴う
  波形の非対称性(スキューネス)は含まないため、往復流だけでは片方向への
  正味の岸向き移動は必ずしも生じない(往復せん断による緩み・沈下・
  法面変形が主体)。
...
def plot_results(snapshots, s: MPMState, out_dir):
    ...
    ax.set_title("沖合養浜マウンドの自重変形+波浪外力による変形 (MPM)")
    ax.set_xlim(0, LX)
    ax.set_ylim(0, LY)
    ax.set_aspect("equal")
...
#   浅水変形に伴う波形の非対称性(スキューネス、沖向きより岸向きの流速が
#   強く短時間なピーク形状)を u_target に第2高調波項(2*wave_omega*t 成分)
#   として加えると、片方向(岸向き)への正味の土砂移動(マウンド全体の
#   岸側への這い上がり)を表現できる。本格的にはXBeach等の外部モデルの
#   出力(水位・波高・底面流速等)を読み込み、時間ステップごとに力へ変換して
#   与える一方向カップリングへ拡張できる(akiya_mpm_nourishment/
#   xbeach_forcing.py が参考になる)。
